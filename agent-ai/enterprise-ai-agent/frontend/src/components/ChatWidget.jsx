...
export default function ChatWidget({ apiBase, tenantId, initialToken, onAuthNeeded }) {
  ...
  useEffect(() => {
    ...
    async function fetchHealth() {
      try {
        ...
        console.warn('health_fetch_failed', err)
      }
    }
    fetchHealth()
  ...
  useEffect(() => {
    ...
    async function fetchKnowledgeStatus() {
      try {
        ...
        console.warn('knowledge_status_fetch_failed', err)
      }
    }
  ...
  if (status === 'login') {
    return <LoginForm onLogin={handleLogin} onAuthNeeded={onAuthNeeded} apiBase={apiBase} loginError={loginError} />
  }
...
function LoginForm({ onLogin, onAuthNeeded, apiBase, loginError }) {
    ...
    <div className="login-root">
      ...
      <form onSubmit={submit} className="login-form">
        ...
        <input
          ...
          required
        />
        {(error || loginError) && <div className="login-error">{error || loginError}</div>}
