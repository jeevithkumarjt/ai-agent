...
│  │  ├─ hero.css                  (M3)
│  │  ├─ carousel.css              (M6)
│  │  ├─ cards.css                 (M6: enterprise card — tags, CTA, hover overlay)
│  │  ├─ orb.css                   (M5 orb slot + play badge)
│  │  ├─ modal.css                 (M7 morph + chat)
│  │  ├─ micro.css                 (M9 hover/press/focus polish)
│  │  ├─ sections.css              (M15: stats, capabilities, industries, features, stories, integrations, pricing, FAQ, CTA, footer)
│  │  ├─ a11y.css                  (M12 focus, reduced-motion, forced-colors)
...
│  │  ├─ background.js             (M2: aurora blobs, noise, cursor light, particles)
│  │  ├─ renderer.js               (renderer layer: card/chrome/dot DOM factory)
│  │  ├─ orb-engine.js             (M5: procedural orb renderer + states)
│  │  ├─ api-adapter.js            (API adapter: GET /solutions → demo, normalization)
│  │  ├─ hero.js                   (M3: entrance stagger, meter, parallax, reveal)
│  │  ├─ carousel.js               (M6: physics cover-flow — DOM via renderer)
...
│  │  └─ app.js                    (M8/M13: async boot via adapter, multi-instance)
│  ├─ svg/icons/                   (M1: icon set + icons.svg sprite)
│  └─ data/demo.json               (M13: demo payload, enterprise contract)
...
## 19a. Enterprise data contract & API adapter (Tryvium architecture alignment)

The frontend stays **fully independent of the backend** — it consumes a JSON
contract and normalizes it to one internal card shape.

**REST flow**

```
Homepage loads  →  GET /solutions  →  JSON  →  adapter.normalizeAll()
                                            →  renderer.card()  →  carousel engine
User clicks a card  →  POST /session (scenario.api)  →  backend  →  voice session
```

**Contract (accepted from the wire, unmodified):**
...
Also accepted: `{ "solutions": [...] }` or a bare array. Demo-only voice fields
(`greeting`, `replies`, `prompts`) are optional — the adapter supplies safe
defaults, so enterprise cards work without them.

**Adapter fallback chain** (`VUI.adapter.load(root)`, first hit wins):

1. `root[data-scenarios]` — JSON injected by the PHP shortcode
2. `window.VoiceUIConfig.scenarios` — `wp_localize_script`
3. `window.VoiceUIConfig.endpoint` — `GET /solutions` (REST)
4. `data/demo.json` — static demo payload
5. built-in demo set — always-present, zero network

**Normalization** maps contract keys to internal card shape
(`industry`/`description`/`tags`/`icon`/`cta`/`api`/`theme`/`orb`, plus voice
fields). `theme` values like `emerald` map to orb palette presets
(`mint`/`sky`/`coral`/`gold`).

**Renderer layer** (`VUI.renderer`) is a pure DOM factory — `card()`,
`chrome()`, `dot()`, `iconBtn()` — with no physics or state. The carousel
engine (physics/input) and the renderer (DOM) are cleanly separated so the
module scales to unlimited cards without rewrites.
